...
<? } ?>
		...
		<form method="post" action="<?=SITEROOT?>/ucp/process/changeInfo">
<!--			<div class="tr">
			...
			<div class="tr">
				<div><button type="submit" name="submit" class="btn_submit"></button></div>
			</div>
		</form>
		
		<hr>
		<h2>User Details</h2>
		<form id="changeOptions" method="post" action="<?=SITEROOT?>/ucp/process/changeDetails" enctype="multipart/form-data">
			<div class="tr">
				<label class="textLabel">Profile Image</label>
<? if (file_exists(FILEROOT."/ucp/avatars/$userID.jpg")) { ?>
				<div id="avatar">
					<img src="<?=SITEROOT?>/ucp/avatars/<?=$userID?>.jpg">
...
<? for ($count = date('Y') - 5; $count >= date('Y') - 100; $count--) echo "						<option".(intval($bdayParts[0]) == $count?' selected="selected"':'').">$count</option>\n"; ?>
		...
		<hr>
		<h2>Forum Options</h2>
		<form id="changeOptions" method="post" action="<?=SITEROOT?>/ucp/process/changeForumOptions">
			<div class="tr">
