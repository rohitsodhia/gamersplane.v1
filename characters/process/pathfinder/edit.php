<?
	...
	if (isset($_POST['save'])) {
		...
		if ($charCheck->rowCount()) {
			...
			$updateSkill = $mysql->prepare("UPDATE pathfinder_skills SET ranks = :ranks, misc = :misc WHERE characterID = :characterID AND skillID = :skillID");
			...
			if (sizeof($_POST['weapons'])) {
				$count = 1;
				$mysql->query('DELETE FROM pathfinder_weapons WHERE characterID = '.$characterID);
				$addWeapon = $mysql->prepare("INSERT INTO pathfinder_weapons (characterID, weaponID, name, ab, damage, critical, `range`, type, size, notes) VALUES (:characterID, :weaponID, :name, :ab, :damage, :critical, :range, :type, :size, :notes)");
			...
			if (sizeof($_POST['armors'])) {
				$count = 1;
				$mysql->query('DELETE FROM pathfinder_armors WHERE characterID = '.$characterID);
				$addArmor = $mysql->prepare("INSERT INTO pathfinder_armors (characterID, armorID, name, ac, maxDex, type, `check`, spellFailure, speed, notes) VALUES (:characterID, :armorID, :name, :ac, :maxDex, :type, :check, :spellFailure, :speed, :notes)");
			...
			$updateChar = $mysql->prepare('UPDATE pathfinder_characters SET '.implode($updates, ', ').' WHERE characterID = :characterID');
