<?
	if (checkLogin(0)) {
		...
		if ($charCheck->rowCount()) {
			...
			$feat = $mysql->query('SELECT featID FROM featsList WHERE name = "'.$name.'"');
			if ($feat->rowCount()) $featID = $feat->fetchColumn();
			else {
			...
			if ($addFeat->getResult()) {
				...
				echo "\t\t\t\t\t\t<a href=\"".SITEROOT."/characters/spycraft2/featNotes/$characterID/$featID?modal=1\" id=\"featNotesLink_$featID\" class=\"feat_notesLink\">Notes</a>\n";
