<?
	...
	$mysql->query("SELECT characterID FROM characters WHERE characterID = $characterID AND userID = $userID");
	if ($mysql->rowCount()) {
		$mysql->query("SELECT featsList.name, dnd3_feats.notes FROM dnd3_feats INNER JOIN featsList USING (featID) WHERE dnd3_feats.featID = $featID AND dnd3_feats.characterID = $characterID");
		if ($mysql->rowCount()) $featInfo = $mysql->getList();
	} else $noChar = TRUE;
?>
...
<? } else { ?>
		...
		<form id="featDescForm" method="post" action="<?=SITEROOT?>/characters/process/dnd3/featNotes/">
