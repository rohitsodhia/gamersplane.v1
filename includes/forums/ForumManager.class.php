	...
	public function __construct($forumID, $options = 0)
	{
		...
		if (bindec($options & $this::NO_CHILDREN) != 0) {
			if ($this->currentForum == 2) {
				$forumsToGet[] = 10;
			} elseif ($this->currentForum == 0) {
				$forumsToGetR = $mysql->query("SELECT h.childID FROM forums_heritage h LEFT JOIN forums_heritage g ON h.childID = g.childID AND g.parentID = 2 WHERE h.parentID = {$this->currentForum} HAVING g.parentID = NULL");
			} else {
