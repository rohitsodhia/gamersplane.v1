<?
	class ForumManager {
		...
		public function __construct($forumID) {
			...
			$forumsR = $mysql->query("SELECT f.forumID, f.title, f.description, f.forumType, f.parentID, f.heritage, f.`order`, f.gameID, f.threadCount FROM forums f INNER JOIN forums p ON p.forumID = {$forumID} AND f.heritage LIKE CONCAT(p.heritage, '%')".($forumID == 0 || $forumID == 2?' WHERE f.heritage NOT LIKE CONCAT(LPAD(2, '.HERITAGE_PAD.', 0), %) OR f.forumID IN (2, 10)':'')." ORDER BY LENGTH(f.heritage)");
