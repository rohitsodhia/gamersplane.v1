...
<?
		...
		public function addTopNotifications(){
			...
			$threadNotifications = $mysql->query("SELECT forumNotifications.notificationType, forumNotifications.postID, forumNotifications.thread threadID, posts.title threadTitle, forums.title forumTitle FROM forumNotifications INNER JOIN threads ON forumNotifications.threadID = threads.threadID INNER JOIN posts ON threads.firstPostID = posts.postID INNER JOIN forums ON threads.forumID = forums.forumID WHERE forumNotifications.userID = {$currentUser->userID}");
