<?
	...
	if (!in_array($_POST['category'], $faqsCategories)) 
		$formErrors->addError('noCategory');
	if (!strlen($_POST['question'])) 
		$formErrors->addError('noQuestion');
	if (!strlen($_POST['answer'])) 
		$formErrors->addError('noAnswer');

	if ($formErrors->errorsExist()) 
		$formErrors->setErrors('addFAQ');
