<?
	...
	if (isset($_POST['delete'])) {
		...
		$permissions = retrievePermissions($currentUser->userID, $postInfo['forumID'], 'deletePost, deleteThread, moderate', TRUE);
 		...
 		if (($permissions['moderate'] || $postInfo['authorID'] == $currentUser->userID) && $postID != $postInfo['firstPostID'] && $permissions['deletePost']) {
			...
			echo 'refresh';
		} elseif (($permissions['moderate'] || $postInfo['authorID'] == $currentUser->userID) && $postID == $postInfo['firstPostID'] && $permissions['deleteThread']) {
