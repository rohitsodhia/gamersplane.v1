<?
	...
	if ($permissions['read']) {
		...
		if ($pollInfo->rowCount()) {
			...
			$mysql->query("SELECT pv.pollOptionID FROM forums_pollVotes pv, forums_pollOptions po WHERE po.threadID = $threadID AND po.pollOptionID = pv.pollOptionID AND pv.userID = $userID");
			if ($mysql->rowCount() > 0 && !$pollInfo['allowRevoting']) {
			} elseif ($mysql->rowCount() == 0 || $pollInfo['allowRevoting']) {
				...
				$validVotes = '';
				if (is_array($_POST['votes']) && sizeof($_POST['votes']) <= $pollInto['optionsPerUser']) {
