<div class="combobox" ng-click="$event.stopPropagation()">
	...
	<div class="results" ng-show="showDropdown">
		<a ng-repeat="set in data | filter: (!bypassFilter || '') && { value: search }" data-id="{{set.id}}" ng-mousedown="setBox(set)" ng-mouseover="setSelected(set, $event)" ng-bind-html="set.value | trustHTML"></a>
	</div>
</div>