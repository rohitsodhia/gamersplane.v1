<div ng-click="$event.stopPropagation()" class="combobox" ng-class="{ 'autocomplete': usingAutocomplete, 'focused': hasFocus }">
	...
	<div class="results" ng-show="showDropdown && filterData().length">
		<a ng-repeat="set in options | filter: (!bypassFilter || '') && { display: search }" ng-mousedown="setBox(set)" ng-mouseover="setSelected($index)" ng-class="set.class.concat({ 'selected': curSelected == $index })" ng-bind-html="set.display | trustHTML"></a>
	</div>
</div>