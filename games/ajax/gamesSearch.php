<?
	...
	$games = $mysql->query("SELECT g.gameID, g.title, g.system, g.gmID, u.username FROM games g LEFT JOIN players p ON g.gameID = p.gameID AND p.userID = {$currentUser->userID} INNER JOIN users u ON g.gmID = u.userID WHERE g.gmID != {$currentUser->userID} AND p.userID IS NULL AND g.status = 1".($fSystems?" AND g.system IN ($fSystems)":'').(isset($search)?" AND ($search)":'')." ORDER BY $order");
