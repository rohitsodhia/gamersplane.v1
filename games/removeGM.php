<?
	...
	if ($mainGMCheck->rowCount() == 0) { header('Location: '.SITEROOT.'/games/'.$gameID); }
	...
	if ($gmInfo->rowCount() == 0) { header('Location: '.SITEROOT.'/games/'.$gameID); }
	$gmInfo = $gmInfo->fetch();
?>
<? require_once(FILEROOT.'/header.php'); ?>
		<h1>Remove Player as GM</h1>
		
		<p class="alignCenter">Are you sure you want to remove <a href="<?=SITEROOT?>/pms/send?userID=<?=$gmInfo['userID']?>" class="username"><?=$gmInfo['username']?></a> as a GM?</p>
		
		<form method="post" action="<?=SITEROOT?>/games/process/removeGM" class="alignCenter">
