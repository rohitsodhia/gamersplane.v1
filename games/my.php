...
<?
	$games = $mysql->query('SELECT g.gameID, g.title, g.status, s.fullName system FROM games g INNER JOIN players p ON g.gameID = p.gameID AND p.isGM = 1 INNER JOIN systems s ON g.system = s.shortName WHERE p.userID = '.$currentUser->userID.' ORDER BY g.status DESC, s.fullName, g.title');
