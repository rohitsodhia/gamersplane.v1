$(function() {
	...
	$('#skills').on('click', '.skill_remove', function (e) {
		...
		$.post('/characters/ajax/sweote/removeSkill', { characterID: characterID, skillID: skillID }, function (data) {
	...
	$('#addWeapon').click(function (e) {
		$.post('/characters/ajax/sweote/weapon', { weaponNum: $('.weapon').size() + 1 }, function (data) { $(data).hide().appendTo('#weapons > div').slideDown(); } );
