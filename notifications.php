...
<?				} elseif ($action == 'charEdited') { ?>
				<div class="text"><?=$cNotification['enactedBy'] == $currentUser->userID?'You':"<a href=\"/ucp/{$cNotification['enactedBy']}/\" class=\"username\">{$cNotification['eUsername']}</a>"?> edited <?=$cNotification['cUserID'] == $currentUser->userID?'your':"<a href=\"/ucp/{$cNotification['cUserID']}/\" class=\"username\">{$cNotification['cUsername']}</a>'s"?> <span class="system"><?=$systems->getFullName($cNotification['system'])?></span> character: <a href="/characters/<?=$cNotification['system']?>/<?=$cNotification['characterID']?>/"><?=$cNotification['label']?></a></div>
...
<?				} elseif ($action == 'charFavorited' || $action == 'charUnfavorited') { ?>
				<div class="text"><?=$cNotification['enactedBy'] == $currentUser->userID?'You':"<a href=\"/ucp/{$cNotification['enactedBy']}/\" class=\"username\">{$cNotification['eUsername']}</a>"?> <?=$action == 'unfavorited'?'un':''?>favorited <?=$cNotification['cUserID'] == $currentUser->userID?'your':"<a href=\"/ucp/{$cNotification['userID']}/\" class=\"username\">{$cNotification['username']}</a>'s"?> <a href="/characters/<?=$cNotification['system']?>/<?=$cNotification['characterID']?>/"><?=$cNotification['label']?></a> (<span class="system"><?=$systems->getFullName($cNotification['system'])?></span>)</div>
<?				} elseif ($action == 'charApplied') { ?>
				<div class="text">You applied <a href="/characters/<?=$cNotification['system']?>/<?=$cNotification['characterID']?>/"><?=$cNotification['label']?></a> (<span class="system"><?=$systems->getFullName($cNotification['system'])?></span>) to <a href="/ucp/<?=$cNotification['gmID']?>/" class="username"><?=$cNotification['gmUsername']?></a>'s game: <a href="/games/<?=$cNotification['gameID']?>?>/"><?=$cNotification['title']?></a></div>
<?				} elseif ($action == 'characterApproved' || $action == 'characterRejected' || $action == 'characterRemoved') { ?>
				<div class="text"><?=$cNotification['enactedBy'] == $currentUser->userID?'You':"<a href=\"/ucp/{$cNotification['enactedBy']}/\" class=\"username\">{$cNotification['eUsername']}</a>"?> <?=strtolower(substr($action, 9))?> <a href="/characters/<?=$cNotification['system']?>/<?=$cNotification['characterID']?>/"><?=$cNotification['label']?></a> (<span class="system"><?=$systems->getFullName($cNotification['system'])?></span>) <?=substr($action, 9) == 'Approved'?'to':'from'?> <?=$cNotification['gmID'] == $currentUser->userID?'your':"<a href=\"/ucp/{$cNotification['gmID']}/\" class=\"username\">{$cNotification['gmUsername']}</a>"?>'s game: <a href="/games/<?=$cNotification['gameID']?>?>/"><?=$cNotification['title']?></a></div>
<?
				}
				echo "			</div>\n";
			} else $count--;
...
<?				} elseif ($action == 'playerApplied') { ?>
				<div class="text"><?=$gNotification['enactedBy'] == $currentUser->userID?'You':"<a href=\"/ucp/{$gNotification['enactedBy']}/\" class=\"username\">{$cNotification['eUsername']}</a>"?> applied to <span class="system"><?=$systems->getFullName($gNotification['system'])?></span> game: <a href="/games/<?=$gNotification['gameID']?>?>/"><?=$gNotification['title']?></a></div>
