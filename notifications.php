...
<?			} elseif ($action == 'playerApproved' || $action == 'playerRejected') { ?>
				<div class="text"><?=$gNotification['enactedBy'] == $currentUser->userID?'You':"<a href=\"/ucp/{$gNotification['enactedBy']}/\" class=\"username\">{$gNotification['eUsername']}</a>"?> <?=strtolower(substr($action, 6))?> <a href="/characters/<?=$systemInfo['shortName']?>/<?=$gNotification['characterID']?>/"><?=$gNotification['label']?></a> (<span class="system"><?=$systemInfo['fullName']?></span>) <?=substr($action, 6) == 'Approved'?'to':'from'?> <?=$gNotification['gmID'] == $currentUser->userID?'your':"<a href=\"/ucp/{$gNotification['gmID']}/\" class=\"username\">{$gNotification['gmUsername']}</a>"?>'s game: <a href="/games/<?=$gNotification['gameID']?>?>/"><?=$gNotification['title']?></a></div>
<?
			} else $count--;
			$gNotification = $gameHistories->fetch();
