...
<?	if ($details['system'] != 'custom') { ?>
			<p style="text-align: center"><img src="<?=$pathBase?>/images/logos/<?=$details['system']?>.png" style="max-width: 300px;"></p>
<?	} ?>
			<p>A new <?=$systems->getFullName($details['system'])?> game has been started on Gamers Plane!</p>
			<p><a href="<?=$pathBase?>/user/<?=$currentUser->userID?>/" class="username"><?=$currentUser->username?></a> has started up a game for <?=$details['numPlayers']?> players called "<?=$details['title']?>". Check out the <a href="<?=$pathBase?>/games/<?=$gameID?>/">game's details</a> and submit a character before the slots fill!</p>
			<p style="text-align: right;">- The Gamers Plane Team</p>
			<p style="font-size: .8em; margin-top: 2em;">If you'd like to stop recieving emails about new games, please head to your <a href="<?=$pathBase?>/ucp/cp/">user control panel</a> and change the option labeled as "Recieve new game emails?" to "No".</p>
		</td></tr>
